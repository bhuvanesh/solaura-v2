...
function generateSQLQuery(deviceMonths) {
    ...
    return `SELECT \`Device ID\`, \`month\`, \`Year\`, \`Actual\`, \`Actual_used\`, \`Estimated_used\`, IFNULL(\`Actual_used\`, 0) AS \`Actual_used\`, IFNULL(\`Estimated_used\`, 0) AS \`Estimated_used\`,IFNULL(\`Actual\`, 0) AS \`Actual\` FROM ${process.env.MASTER_TABLE} WHERE (\`Device ID\`, \`month\`, \`Year\`) IN (${tuples})`;
  }
export async function POST(req) {
  ...
  try {
    ...
    await conn.execute(`UPDATE ${process.env.BUYERS_TABLE} SET \`Status\` = "succeeded" WHERE \`Transaction ID\` = ?`, [transactionId]);
    ...
    if (deviceMonths.length > 0) {
      ...
      for (const inventory2Row of inventory2Rows) {
        ...
        const actual = parseFloat(inventory2Row['Actual']);
