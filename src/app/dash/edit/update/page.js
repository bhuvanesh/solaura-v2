...
const FormPage = () => {
  ...
  const rowData = searchParams ? Object.fromEntries(searchParams) : null;
  console.log('incoming DataTransfer',rowData);
// Update the form state with the rowData
useEffect(() => {
    if (rowData) {
      const codDate = new Date(rowData.CoD);
      const formattedCodDate = `${codDate.getFullYear()}-${String(codDate.getMonth() + 1).padStart(2, '0')}-${String(codDate.getDate()).padStart(2, '0')}`;
  
      setFormData({
        groupName: rowData.Group,
        companyName: rowData.company,
        projectName: rowData.project,
        capacity: rowData['Capacity (MW)'],
        deviceId: rowData['Device ID'],
        cod: formattedCodDate,
        deviceType: rowData.Type,
        months: [
          rowData.January,
          rowData.February,
          rowData.March,
