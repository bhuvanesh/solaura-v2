...
const sql = `
      SELECT 
        SUM(CASE WHEN Month = MONTHNAME(DATE_SUB(CURRENT_DATE, INTERVAL 2 MONTH)) AND Year = YEAR(CURRENT_DATE) - 1 THEN Actual ELSE 0 END) AS prev_month_actual_generation_previous_year,
        SUM(CASE WHEN Month = MONTHNAME(DATE_SUB(CURRENT_DATE, INTERVAL 2 MONTH)) AND Year = YEAR(CURRENT_DATE) THEN Actual ELSE 0 END) AS prev_month_actual_generation,
