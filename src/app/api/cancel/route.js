...
for (const [month, valueToSubtract] of Object.entries(monthData)) {
  ...
  const [[inventory2Row]] = await conn.query(`SELECT \`Actual_used\`, \`Estimated_used\`, COALESCE(\`Actual_used\`, 0) AS \`Actual_used\`, COALESCE(\`Estimated_used\`, 0) AS \`Estimated_used\` FROM ${process.env.MASTER_TABLE} WHERE \`Device ID\` = ? AND \`month\` = ? AND \`Year\` = ?`, [deviceId, lowercaseMonth, year]);
  ...
  if (inventory2Row) {
    ...
    if (actualUsed > 0) {
      await conn.query(`UPDATE ${process.env.MASTER_TABLE} SET \`Actual_used\` = COALESCE(\`Actual_used\`, 0) - ? WHERE \`Device ID\` = ? AND \`month\` = ? AND \`Year\` = ?`, [valueToSubtract, deviceId, lowercaseMonth, year]);
    } else {
      await conn.query(`UPDATE ${process.env.MASTER_TABLE} SET \`Estimated_used\` = COALESCE(\`Estimated_used\`, 0) - ? WHERE \`Device ID\` = ? AND \`month\` = ? AND \`Year\` = ?`, [valueToSubtract, deviceId, lowercaseMonth, year]);
    }
  }
}
